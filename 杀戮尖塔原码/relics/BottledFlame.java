...
// Source File Name:   BottledFlame.java
...
public class BottledFlame extends AbstractRelic
{
    ...
    public void onEquip()
    {
        if(AbstractDungeon.player.masterDeck.getPurgeableCards().getAttacks().size() > 0)
        {
            ...
            AbstractDungeon.gridSelectScreen.open(AbstractDungeon.player.masterDeck.getPurgeableCards().getAttacks(), 1, (new StringBuilder()).append(DESCRIPTIONS[1]).append(name).append(LocalizedStrings.PERIOD).toString(), false, false, false, false);
        }
    }
    ...
    public boolean canSpawn()
    {
        for(Iterator iterator = AbstractDungeon.player.masterDeck.group.iterator(); iterator.hasNext();)
        {
            ...
            if(c.type == com.megacrit.cardcrawl.cards.AbstractCard.CardType.ATTACK && c.rarity != com.megacrit.cardcrawl.cards.AbstractCard.CardRarity.BASIC)
                return true;
        }
