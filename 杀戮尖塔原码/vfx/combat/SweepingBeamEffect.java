...
// Source File Name:   SweepingBeamEffect.java
...
public class SweepingBeamEffect extends AbstractGameEffect
{
    ...
    public void update()
    {
        ...
        if(isFlipped)
        {
            dX = ((float)Settings.WIDTH / 2.0F) * Interpolation.pow3Out.apply(duration);
            dY = AbstractDungeon.floorY + 10F * Settings.scale;
        } else
        {
            dX = (float)Settings.WIDTH + ((float)(-Settings.WIDTH) / 2.0F) * Interpolation.pow3Out.apply(duration);
            dY = AbstractDungeon.floorY + 30F * Settings.scale;
        }
        dst = Vector2.dst(sX, sY, dX, dY) / Settings.scale;
        ...
        if(duration > startingDuration / 2.0F)
            ...
            color.a = Interpolation.pow2Out.apply(0.0F, 1.0F, duration * 4F);
        if(duration < 0.0F)
            isDone = true;
    }
