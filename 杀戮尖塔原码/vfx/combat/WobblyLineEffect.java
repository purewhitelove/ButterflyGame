...
// Source File Name:   WobblyLineEffect.java
...
public class WobblyLineEffect extends AbstractGameEffect
{
    ...
    public WobblyLineEffect(float x, float y, Color color)
    {
        ...
        scale = MathUtils.random(0.2F, 0.4F);
        this.x = x - (float)img.packedWidth / 2.0F;
        this.y = y - (float)img.packedHeight / 2.0F;
        duration = 2.0F;
        this.color = color;
        renderBehind = MathUtils.randomBoolean();
        speedStart = MathUtils.random(300F, 1000F) * Settings.scale;
        speedTarget = 900F * Settings.scale;
        speed = speedStart;
    ...
    public void render(SpriteBatch sb)
    {
        ...
        sb.setColor(color);
        sb.draw(img, x, y, (float)img.packedWidth / 2.0F, (float)img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, scale + MathUtils.random(-0.08F, 0.08F) * Settings.scale, scale + MathUtils.random(-0.08F, 0.08F) * Settings.scale, rotation + flipper + MathUtils.random(-5F, 5F));
