...
// Source File Name:   RedFireBurstParticleEffect.java
...
public class RedFireBurstParticleEffect extends AbstractGameEffect
{
    ...
    public RedFireBurstParticleEffect(float x, float y, int timesUpgraded)
    {
        ...
        this.y = (y - (float)(img.packedHeight / 2)) + MathUtils.random(-20F, 20F) * Settings.scale;
        color = new Color(MathUtils.random(0.8F, 1.0F), MathUtils.random(0.3F, 0.8F), MathUtils.random(0.1F, 0.2F), 0.0F);
        color.a = 0.0F;
        rotation = MathUtils.random(-10F, 10F);
        scale = MathUtils.random(1.5F, 2.0F);
        scale += (float)timesUpgraded * 0.1F;
        scale *= Settings.scale;
        vX = MathUtils.random(-900F, 900F) * Settings.scale;
        vY = MathUtils.random(0.0F, 300F) * Settings.scale;
