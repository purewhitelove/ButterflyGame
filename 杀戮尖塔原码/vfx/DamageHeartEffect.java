...
// Source File Name:   DamageHeartEffect.java
...
public class DamageHeartEffect extends AbstractGameEffect
{
    ...
    public DamageHeartEffect(float delay, float x, float y, com.megacrit.cardcrawl.actions.AbstractGameAction.AttackEffect effect, int damage)
    {
        triggered = false;
        delayTimer = 2.0F;
        duration = 0.6F;
        ...
        this.damage = damage;
        this.x = (x - (float)img.packedWidth / 2.0F) + MathUtils.random(-150F, 150F) * Settings.scale;
        y -= (float)img.packedHeight / 2.0F + MathUtils.random(-150F, 150F) * Settings.scale;
        static class _cls1
        {
    ...
    public void update()
    {
        if(delayTimer > 0.0F)
        {
            ...
            {
                playSound(effect);
                AbstractDungeon.effectsQueue.add(new StrikeEffect(null, x + (float)img.packedWidth / 2.0F, y + (float)img.packedHeight / 2.0F, damage));
            }
            return;
        }
        switch(_cls1..SwitchMap.com.megacrit.cardcrawl.actions.AbstractGameAction.AttackEffect[effect.ordinal()])
